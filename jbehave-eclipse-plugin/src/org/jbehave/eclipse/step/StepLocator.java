...
public class StepLocator {
    ...
    public Iterable<WeightedCandidateStep> findCandidatesStartingWith(final String stepLine) {
        ...
        try {
            ...
            final String searchedType = StepParser.stepType(localizedStepSupport, stepLine);
            final String stepEntry = StepParser.extractStepSentence(localizedStepSupport, stepLine);
    ...
    private PotentialStep getFirstStepWithHighestPrio(Iterable<PotentialStep> potentialSteps) {
        fj.data.List<Integer> collectedPrios = iterableList(potentialSteps).map(new PotentialStepTransformer());
        if (collectedPrios.isEmpty()) {
            return null;
        }
        
        final int maxPrio = collectedPrios.maximum(Ord.intOrd);
        fj.data.List<PotentialStep> maxPrioSteps = iterableList(potentialSteps).filter(new PotentialStepFilter(maxPrio));
